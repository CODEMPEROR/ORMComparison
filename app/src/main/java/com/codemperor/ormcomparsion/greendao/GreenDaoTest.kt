...
import com.codemperor.ormcomparsion.dbflow.DBFlowApplication
...
class GreenDaoTest private constructor() {
    ...
    fun test(ctx: Context, handler: CallbackHandler) {
        ...
        object : Thread() {
            override fun run() {
                ...
                val res = "Test GreenDAO " + testTimes + " times, Average Time : " + timeCounter.toString()
                Log.i(TAG, res)
                val msg = handler.obtainMessage()
                msg.what = MainActivity.MSG_WHAT_GREENDAO;
                msg.obj = res;
    ...
    private fun init() {
// init GreenDAO
        val helper = DaoMaster.DevOpenHelper(mContext, DBFlowApplication.getUserId() + "greendao-notes", null)
